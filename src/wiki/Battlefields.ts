	...
	for (const battlefield of Battlefield.getAllGenerator()) {
		const ul = [
			...
			...(battlefield.rankGroupId ? [
				`排名獎勵：${wikiPageLink("BattlefieldRanks", "", battlefield.rankGroupId)}`,
				`排名蒐集品：${battlefield.targetItem?.toWiki() ?? ""}`,
			] : []),
			...(battlefield.evaluateGroupId ? [
				`戰鬥評價：${wikiPageLink("Evaluates", "", battlefield.evaluateGroupId)}`,
				`評價成就：${wikiPageLink("EvaluateAchievements", "", battlefield.evaluateAchievementGroupId)}`,
			] : []),
			...(battlefield.pointGroupId ? [
				`積分獎勵：${wikiPageLink("AdventureRank", "", `積分獎勵 ${battlefield.pointGroupId}`)}`
			] : []),
			`章節：${wikiul(battlefield.chapters.map(chapter => wikiPageLink("Chapter", chapter.getWikiFullName(), chapter.id)))}`,
		];
