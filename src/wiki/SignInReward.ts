...
export default function wikiSignInReward() {
	...
	out += `\n\n${wikiH2(localizationString("Metagame")("signInRule"))}\n<pre>${localizationString("Metagame")("signInCardDescription")}</pre>`;

	const groups = _.groupBy(SignInRewardTable.rows, (r) => r.get("groupId"));
	for (const [groupId, group] of Object.entries(groups)) {
		const categories = _.groupBy(group.sort((a, b) => a.get("param1") - b.get("param1")), (r) => r.get("category"));
		...
		if (categories.month) {
			const table: WikiTableStruct = categories.month.map((row): WikiTableRow => [
				{
					header: true,
			...
			out += `\n${wikiH3(localizationString("Metagame")("signInMonthReward"), `${groupId}_month`)}\n${wikitable(table)}`;
		}
		const otherCategories = Object.keys(categories).filter(key => !['day', 'week', 'month'].includes(key));
		for (const category of otherCategories) {
			const name = category == "special" ? localizationString("Metagame")("signInSpecial") : category;
			out += `\n${wikiH3(name, `${groupId}_${category}`)}\n${wikitable(buildWeekTable(categories[category]))}`;
		}
	}
...
function buildWeekTable(days: RowWrapper[], weeks?: RowWrapper[]): WikiTableStruct {
	...
	for (let i = 0, weekIndex = 0; i < days.length; i += 7, weekIndex++) {
		...
		table.rows.push({
			...
			...range(0, 6).map(j => days[i + j] ? item2wikiWithType(days[i + j].get("giveType"), days[i + j].get("giveLinkId"), days[i + j].get("giveAmount"), {
				direction: "vertical"
			}) : ""),
			...(weekRow ? [
