...
export default function wikiAdventureRank() {
	...
	for (const [groupId, group] of Object.entries(dailyRankGroups)) {
		...
		for (let i = 0; i < group.length; i++) {
			const entry = group[i];
			table.rows.push([
				`${wikiimage(dailyRankImage[entry.get("image")], { width: 64 })} (${entry.get("maxPercentage")}% ~ ${entry.get("minPercentage")}%)`,
			...
			table.rows.push([
				{
					...
					text: wikiimage(weekRankImage[entry.get("image")], { width: 64 }),
				},
				{
					attributes: `style="text-align: center;"`,
					...
					...(index === 0 ? [
						{
							...
							text: wikiNextLine(`${wikiimage(`幻境${rankNameCh}_Icon.png`, { width: 30 })}\n${rankNameCh}`),
						},
					] : []),
					{
						attributes: `style="text-align: center;"`,
	...
	out += `\n\n${wikiH2("積分獎勵")}`;
	const weekPointGroups = _.groupBy(
		AdventureWeekPointTable.rows,
		(r) => r.get("groupId")
	);
