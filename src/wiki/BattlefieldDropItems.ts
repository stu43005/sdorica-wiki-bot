		...
		for (const entry of group) {
			table.push([
				...
				...range(1, 4).map((i): WikiTableCeil => ({
					attributes: entry.get(`giveLinkId${i}`) != lv1.get(`giveLinkId${i}`) ? `style="background-color: #ffd700; color: #1e1e1e;"` : "",
					text: `${item2wikiWithType(entry.get(`giveType${i}`), entry.get(`giveLinkId${i}`), entry.get(`giveAmount${i}`))}${entry.get(`chance${i}`) != 10000 && entry.get(`chance${i}`) != -1 ? `：${entry.get(`chance${i}`) / 100}%` : ""}`,
				})),
			]);
		}
		out += `\n\n${wikiH2(groupId)}\n${wikitable(table)}`;
