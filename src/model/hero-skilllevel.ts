...
export class HeroSkillLevel implements IHeroSkillSet {
	...
	public static get(rowOrId: RowWrapper | string): HeroSkillLevel {
		...
		if (!instances[id]) {
			const row = typeof rowOrId === 'string' ? SkillLevelTable.find(r => r.get('id') == id) : rowOrId;
			if (row) {
	...
	get hero() { return this.rootSkillSet?.hero; }
	get requiredSubrank(): number { return +this.row.get('requiredSubrank'); }
	...
	get tipsA1(): boolean { return !!this.row.get('tipsA1'); }
	...
	constructor(private row: RowWrapper) {
		...
		this.payItems = [];
		if (row.get('pay1Type')) this.payItems.push(new ItemPayRef(row.get('pay1Type'), row.get('pay1LinkId'), row.get('pay1Amount')));
		if (row.get('pay2Type')) this.payItems.push(new ItemPayRef(row.get('pay2Type'), row.get('pay2LinkId'), row.get('pay2Amount')));
		if (row.get('pay3Type')) this.payItems.push(new ItemPayRef(row.get('pay3Type'), row.get('pay3LinkId'), row.get('pay3Amount')));
		if (row.get('pay4Type')) this.payItems.push(new ItemPayRef(row.get('pay4Type'), row.get('pay4LinkId'), row.get('pay4Amount')));
	}
