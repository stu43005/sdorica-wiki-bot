...
export class Mission {
	...
	public static get(rowOrId: RowWrapper | string): Mission {
		...
		if (!instances[id]) {
			const row = typeof rowOrId === 'string' ? MissionsTable.find(r => r.get('id') == id) : rowOrId;
			if (row) {
	...
	get type(): MissionType { return this.row.get('type'); }
	get enable(): boolean { return !!this.row.get('enable'); }

	get minLv(): number { return +this.row.get('minLv'); }
	get maxLv(): number { return +this.row.get('maxLv'); }

	/**
	 * @deprecated 改用 {@link getMissionName}
	 */
	get name(): string { return this.row.get('name'); }

	get param1(): number { return +this.row.get('param1'); }
	get param2(): number { return +this.row.get('param2'); }
	get param3(): string { return this.row.get('param3'); }
	get requireId(): string { return this.row.get('requireId'); }

	#giveItem: ItemGiveRef | null = null;
	get giveItem(): ItemGiveRef {
		return this.#giveItem ??= new ItemGiveRef(this.row.get("giveType"), this.row.get("giveLinkId"), this.row.get("giveAmount"));
	}

	get timeLimit(): number { return +this.row.get('timeLimit'); }
	get weight(): number { return +this.row.get('weight'); }
	...
	public getMissionName(wikilink = false): string {
		...
		const limitations = this.param3.split(";").map(limit => {
			...
			if (limit.startsWith("LimitDead")) {
				...
				return new TemplateString(localizationString("Mission")("limit_casualties_count")).apply({
					casualty: count,
				});
			}
			...
			if (limit.startsWith("CastMore")) {
				...
				if (match) {
					...
					if (skill.startsWith("S")) {
						...
						if (block) {
							return new TemplateString(localizationString("Mission")("castmore_Skill")).apply({
								block,
								skillCount,
							});
						}
					}
					return new TemplateString(localizationString("Mission")(`castmore_${skill}`)).apply({
						skillCount,
					});
				}
				debugger;
			...
			if (limit.startsWith("CastLess")) {
				...
				if (match) {
					...
					if (skill.startsWith("S")) {
						...
						if (block) {
							if (!skillCount) {
								return new TemplateString(localizationString("Mission")("cast_no_erase_Skill")).apply({
									block,
								});
							}
							return new TemplateString(localizationString("Mission")("castless_Skill")).apply({
								block,
								skillCount,
							});
						}
					...
					return new TemplateString(localizationString("Mission")(`castless_${skill}`)).apply({
						skillCount,
					});
				}
				debugger;
		...
		if (quest && quest.chapter) {
			const questname = wikilink ? wikiPageLink(quest.getWikiLink(), quest.title) : quest.title;
			switch (quest.chapter.group) {
				...
				case ChapterGroup.Challenge:
					clear = new TemplateString(localizationString("Mission")("clear_challenge")).apply({
						questname,
					});
					break;
			}
