...
export class ChapterCount {
		...
		if (!instances[id]) {
			const row =
				typeof rowOrId === "string"
					? ChapterCountTable.find((r) => r.get("id") == id)
					: rowOrId;
			if (row) {
	...
	get dynamicRate(): boolean {
		return !!this.row.get("dynamicRate");
	}

	get initial(): number {
		return +this.row.get("initial");
	}
	/**
	 * 累積最大關卡次數
	 */
	get max(): number {
		return +this.row.get("max");
	}

	get regainType(): ChapteCountType {
		return this.row.get("regainType");
	}
	get regainValue(): number {
		return +this.row.get("regainValue");
	}
	/**
	 * 自動恢復關卡次數
	 */
