...
export default async function (helper: ViewerJSHelper) {
	...
	async function omg(typename: string) {
		const modelRegexp = /(b|e|h|t|m|r|n)\d{4}/gi;
		const data = {
			ab_add: <string[]>[],
			ab_change: <string[]>[],
			ab_delete: <string[]>[],
		...
		const { data: diffData }: { data: SiDiffData } = await vue.$http.get(
			"/api/imperium/ab_diff/",
			{
				params: {
					old: old_id,
					new: new_id,
	...
	const data = {
		...
		ab_add: data1.ab_add
			...
			.sort()
			.map((p) => `ab add: ${p}`),
		ab_change: data1.ab_change
			.concat(data2.ab_change)
			.filter(filter)
			.sort()
			.map((p) => `ab change: ${p}`),
		ab_delete: data1.ab_delete
			.concat(data2.ab_delete)
			.filter(filter)
			.sort()
			.map((p) => `ab delete: ${p}`),
		add: data1.add
		...
		delete: data1.delete
			...
			.sort()
			.map((p) => `delete: ${p}`),
		"add&delete": data1["add&delete"]
			.concat(data2["add&delete"])
			.filter(filter)
			.sort()
			.map((p) => `add & delete: ${p}`),
	};
