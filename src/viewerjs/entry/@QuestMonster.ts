...
export default async function (helper: ViewerJSHelper) {
	...
	try {
		...
		if (isLoadEnemyData) {
			const allEnemies = enemysetDatas
				.map((enemySetData) => {
					return enemySetData.enemyList.map((e) => {
						...
						if (
							e.character.type == 1 &&
							levelactorData &&
							levelactorData.ActorCharacterTable &&
							e.character.name in levelactorData.ActorCharacterTable
						) {
		...
		for (let i = 0; i < wavesets.length; i++) {
			...
			for (let j = 0; j < enemySets.length; j++) {
				...
				const enemies = enemySetData.enemyList.map((e) => {
					...
					return `${name} (${
						localizationCharacterName()(name)
							? localizationCharacterName()(name)
							: localizationCharacterName()(name.split("_")[0])
					}) (level: ${e._level}, initHpRatio: ${e._initHpRatio}, initArmorRatio: ${
						e._initArmorRatio
					}, initialCdBase: ${e._initialCdBase}${
						charAsset && charAsset.$interpreted
							? `, 血量: ${charAsset.$interpreted.血量}, 疊盾: ${charAsset.$interpreted.疊盾}, 攻擊力: ${charAsset.$interpreted.攻擊力}`
							: ""
					})`;
				});
