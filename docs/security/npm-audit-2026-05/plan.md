...
並檢查（若檔案存在）內容是否同時包含 `mwbot.user` 與 `mwbot.pass` 欄位（兩者都必須存在才能成功登入）：

```bash
node -e "try { const c = require('./config/local.json'); console.log(c.mwbot && c.mwbot.user && c.mwbot.pass ? 'V4_ENABLED=true' : 'V4_ENABLED=false'); } catch(e) { console.log('V4_ENABLED=false'); }"
```
...
> **Retry 路徑說明**：若這是 plan 的 retry（已部分完成 Task 1-6），**不要**重跑 P2；直接設 `V5_BASELINE_AVAILABLE=false` 並在 verification-report.md 註明 baseline 已於初次執行時取得（路徑：`/tmp/audit-baseline-assets.json`）。下方步驟僅在初次執行時跑。

第一步：確認 working tree 乾淨（避免 baseline 採集影響中途工作）：

```bash
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "ERROR: working tree not clean. Pre-flight Check P2 must run before any Task starts."
  echo "If this is a retry after partial run, set V5_BASELINE_AVAILABLE=false manually and skip P2."
  exit 1
fi
```

第二步：採集 baseline（若 `data/` 不存在或 build 失敗，標記 `V5_BASELINE_AVAILABLE=false`，後續 Task 7 Step 5 跳過並在 verification-report.md 註記）：

```bash
if [ ! -d data ]; then
  echo "V5_BASELINE_AVAILABLE=false (no data/ directory)"
else
  npm run assetbundle:dev || { echo "V5_BASELINE_AVAILABLE=false (assetbundle build failed)"; exit 0; }

  # 找出剛產生的 assets.json（取最新一份；若有多份，目前 plan 僅追蹤其中一份）
  BASELINE_SRC="$(find . -name 'assets.json' -newer package.json -not -path './node_modules/*' -print 2>/dev/null | head -1)"
  if [ -z "$BASELINE_SRC" ]; then
    echo "V5_BASELINE_AVAILABLE=false (no assets.json produced)"
  else
    cp "$BASELINE_SRC" /tmp/audit-baseline-assets.json
    echo "$BASELINE_SRC" > /tmp/audit-baseline-assets.path
    echo "V5_BASELINE_AVAILABLE=true (baseline at /tmp/audit-baseline-assets.json; source path saved to /tmp/audit-baseline-assets.path for Task 7 Step 5)"
  fi
fi
```
...
驗證：把升級後產生的 `assets.json` 與 P2 取得的 baseline 做 diff（升級後輸出路徑直接從 P2 保存的 `/tmp/audit-baseline-assets.path` 讀取，無需手動填入）：

```bash
NEW_ASSETS="$(cat /tmp/audit-baseline-assets.path)"
diff /tmp/audit-baseline-assets.json "$NEW_ASSETS"
```
